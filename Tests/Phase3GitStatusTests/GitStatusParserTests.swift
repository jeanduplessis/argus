...
struct GitStatusParserTests {
    ...
    private static func parsesBranchMetadataForCleanRepository() {
        let output = """
...
"""
    ...
    private static func parsesChangedFilesIntoSections() {
        let output = """
# branch.head main
...
"""
        ...
        assertEqual(status.stagedFiles.map(\.path), ["staged.txt", "deleted.txt"], "staged paths parse")
    ...
    private static func parsesRenamedAndCopiedFilesWithOriginalPaths() {
        let output = """
# branch.head main
...
"""
        ...
        assertEqual(status.stagedFiles.map(\.path), ["renamed folder/new file.txt", "copied file.txt", "moved and edited.txt"], "renamed and copied staged paths parse")
        assertEqual(status.stagedFiles.map(\.originalPath), ["old folder/old file.txt", "template file.txt", "old edited.txt"], "renamed and copied original paths parse")
        assertEqual(status.stagedFiles.map(\.status), [.renamed, .copied, .renamed], "renamed and copied staged statuses parse")
        assertEqual(status.unstagedFiles.map(\.path), ["moved and edited.txt"], "renamed file with worktree edits appears in unstaged section")
        assertEqual(status.unstagedFiles.first?.originalPath, "old edited.txt", "unstaged rename companion preserves original path")
        assertEqual(status.unstagedFiles.first?.status, .modified, "unstaged rename companion status parses")
    }

    private static func parsesTypeChangedFiles() {
        let output = """
# branch.head main
...
"""
        ...
        assertEqual(status.unstagedFiles, [
            GitFileChange(path: "conflict folder/file with spaces.txt", status: .unmerged, sectionKey: "unstaged")
        ], "unmerged path with spaces parses")
    }

    private static func capsDisplayedFileRowsAtLimit() {
        let output = (0..<501)
            .map { "? file-\($0).txt" }
            .joined(separator: "\n")
        ...
        assertEqual(status.untrackedFiles.count, GitStatusSummary.displayFileLimit, "display rows are capped")
