...
struct GitPreviewServiceTests {
    ...
    private static func runsBlamePreviewWithColorizedOutput() async throws {
        ...
        let service = GitPreviewService(commandBuilder: GitPreviewCommandBuilder(difftasticPathProvider: { nil }))
        let result = await service.preview(kind: .blame, rootPath: repo.url.path, file: GitFileChange(path: "file.txt", status: .modified, sectionKey: "unstaged"))
        ...
        assertEqual(preview.output.contains("Argus Test"), true, "blame output includes git blame content")
        assertEqual(preview.output.contains("\u{001B}["), true, "blame output is colorized")
    }

    private static func reportsPreviewCommandFailureWithoutThrowing() async {
        let service = GitPreviewService(commandBuilder: GitPreviewCommandBuilder(difftasticPathProvider: { nil }))
    ...
    private static func runsDiffPreviewWithColorizedOutput() async throws {
        ...
        let service = GitPreviewService(commandBuilder: GitPreviewCommandBuilder(difftasticPathProvider: { nil }))
        let result = await service.preview(kind: .diff, rootPath: repo.url.path, file: GitFileChange(path: "file.txt", status: .modified, sectionKey: "unstaged"))
    ...
    private static func selectsDiffCommandsForFileSections() {
        ...
        assertEqual(staged?.arguments, ["-C", rootPath, "diff", "--no-ext-diff", "--color=always", "--cached", "--", "Sources/App.swift"], "staged diff uses cached diff from status root")
        ...
        assertEqual(unstaged?.arguments, ["-C", rootPath, "diff", "--no-ext-diff", "--color=always", "--", "Sources/App.swift"], "unstaged diff uses working-tree diff from status root")
        ...
        assertEqual(untracked?.arguments, ["-C", rootPath, "diff", "--no-ext-diff", "--no-index", "--color=always", "--", "/dev/null", "Scratch.txt"], "untracked diff compares file against empty input")
    }

    private static func selectsBlameCommandOnlyForTrackedRows() {
        ...
        assertEqual(tracked?.arguments, ["-C", rootPath, "blame", "--color-lines", "--color-by-age", "--", "Sources/App.swift"], "tracked blame uses colorized git blame from status root")
    ...
    private static func usesDifftasticForDiffOnlyWhenAvailable() {
        let rootPath = "/tmp/repo"
        let file = GitFileChange(path: "Sources/App.swift", status: .modified, sectionKey: "unstaged")

        let withDifftastic = GitPreviewCommandBuilder(difftasticPathProvider: { "/opt/homebrew/bin/difft" })
            .command(kind: .diff, rootPath: rootPath, file: file)
        assertEqual(
            withDifftastic?.arguments,
            ["-C", rootPath, "-c", "diff.external=/opt/homebrew/bin/difft", "diff", "--color=always", "--", "Sources/App.swift"],
