...
struct GitStatusViewModelTests {
    ...
    @MainActor
    private static func manualRefreshPublishesLoadingThenLoadedState() async {
        let service = FakeStatusService(result: .loaded(GitStatusSummary(
    ...
    @MainActor
    private static func initializingRepositoryPublishesCleanRefreshedStatus() async {
        let loaded = GitStatusLoadState.loaded(GitStatusSummary(
        ...
        let service = FakeStatusService(result: .notRepository(rootPath: "/tmp/new-repo"), initializeResult: loaded)
        let viewModel = GitStatusViewModel(service: service)
        let context = GitStatusRootContext(
            kind: .standalone,
    ...
    @MainActor
    private static func copyPathWritesDisplayedFilePathWithoutGitMutation() async {
        ...
        assertEqual(clipboard.copiedPaths, ["Sources/App/File.swift"], "copy-path copies the displayed row path")
    ...
    @MainActor
    private static func fileOperationUsesResolvedRootAndPublishesRefreshedStatus() async {
        let refreshed = GitStatusLoadState.loaded(GitStatusSummary(
            rootPath: "/tmp/worktree",
            branchName: "main",
            upstreamName: nil,
            aheadCount: 0,
        ...
        assertEqual(service.operationRequests.map { "\($0.0)-\($0.1)-\($0.2)" }, ["stage-/tmp/worktree-file.txt"], "file operation uses resolved root and row path")
        assertEqual(viewModel.state, refreshed, "file operation publishes refreshed status immediately")
    }

    @MainActor
    private static func canceledDestructiveFileOperationDoesNotMutateState() async {
        ...
        assertEqual(service.bulkOperationRequests.isEmpty, true, "canceled destructive operation does not call git")
        assertEqual(viewModel.state, .idle, "canceled destructive operation leaves sidebar state unchanged")
    }

    @MainActor
    private static func confirmedDestructiveFileOperationRunsAndRefreshes() async {
        let refreshed = GitStatusLoadState.loaded(GitStatusSummary(
            rootPath: "/tmp/repo",
            branchName: "main",
            upstreamName: nil,
        ...
        await viewModel.confirmAndPerformFileOperation(.delete, paths: ["scratch.txt"], context: context)

        assertEqual(confirmation.requests, [.delete], "confirmed delete asks for confirmation")
        assertEqual(service.bulkOperationRequests.map { "\($0.0)-\($0.1)-\($0.2.joined(separator: ","))" }, ["delete-/tmp/repo-scratch.txt"], "confirmed delete runs as a bulk operation")
        assertEqual(viewModel.state, refreshed, "confirmed destructive operation publishes refreshed status")
    }

    @MainActor
    private static func sectionBulkOperationUsesSectionScopeForCappedResults() async {
        let refreshed = GitStatusLoadState.loaded(GitStatusSummary(
            rootPath: "/tmp/repo",
            branchName: "main",
            upstreamName: nil,
        ...
        await viewModel.confirmAndPerformSectionFileOperation(.stage, sectionKey: "untracked", pathCount: 501, context: context)

        assertEqual(service.bulkOperationRequests.isEmpty, true, "capped section bulk actions do not operate only on displayed file paths")
        assertEqual(service.sectionOperationRequests.map { "\($0.0)-\($0.1)-\($0.2)" }, ["stage-/tmp/repo-untracked"], "bulk action operates on the whole git section")
        assertEqual(viewModel.state, refreshed, "section bulk operation publishes refreshed status")
    }

    @MainActor
    private static func destructiveSectionBulkOperationConfirmsTotalSectionCount() async {
        let service = FakeStatusService(result: .idle)
        let confirmation = RecordingFileOperationConfirmer(shouldConfirm: true)
        let viewModel = GitStatusViewModel(service: service, fileOperationConfirmer: confirmation)
        let context = GitStatusRootContext(
        ...
        await viewModel.confirmAndPerformSectionFileOperation(.delete, sectionKey: "untracked", pathCount: 501, context: context)

        assertEqual(confirmation.requests, [.delete], "destructive section action asks for confirmation")
        assertEqual(confirmation.pathCountRequests, [501], "destructive section confirmation uses total section count, not capped row count")
        assertEqual(service.sectionOperationRequests.map { "\($0.0)-\($0.1)-\($0.2)" }, ["delete-/tmp/repo-untracked"], "confirmed destructive section action operates on whole section")
    }

    @MainActor
    private static func previewUsesResolvedRootAndPresentsOutput() async {
        let service = FakeStatusService(result: .idle)
        let previewService = RecordingPreviewService(result: .loaded(GitPreview(kind: .diff, path: "file.txt", output: "diff output")))
        let presenter = RecordingPreviewPresenter()
        let viewModel = GitStatusViewModel(service: service, previewService: previewService, previewPresenter: presenter)
        let context = GitStatusRootContext(
            kind: .worktree,
            currentDirectory: "/tmp/worktree/subdir",
            worktreePath: "/tmp/worktree",
        ...
        assertEqual(previewService.requests.map { "\($0.0)-\($0.1)-\($0.2.path)" }, ["diff-/tmp/worktree-file.txt"], "preview uses resolved status root and selected row")
        assertEqual(presenter.presentedPreviews, [GitPreview(kind: .diff, path: "file.txt", output: "diff output")], "loaded preview is presented")
        assertEqual(viewModel.state, .idle, "preview does not replace git status state")
    }

    @MainActor
    private static func previewFailureIsPresentedWithoutReplacingStatusState() async {
        let current = GitStatusLoadState.loaded(GitStatusSummary(rootPath: "/tmp/repo", branchName: "main", upstreamName: nil, aheadCount: 0, behindCount: 0))
        let service = FakeStatusService(result: current)
        let previewService = RecordingPreviewService(result: .failed(kind: .blame, path: "missing.txt", message: "fatal: no such path"))
        let presenter = RecordingPreviewPresenter()
        let viewModel = GitStatusViewModel(service: service, previewService: previewService, previewPresenter: presenter)
        let context = GitStatusRootContext(kind: .standalone, currentDirectory: "/tmp/repo", worktreePath: nil, projectRepositoryPath: nil)
        await viewModel.refresh(context: context)

        await viewModel.showPreview(kind: .blame, file: GitFileChange(path: "missing.txt", status: .modified, sectionKey: "unstaged"), context: context, parentWindow: nil)

        assertEqual(presenter.presentedFailures, ["blame-missing.txt-fatal: no such path"], "preview failure is presented non-destructively")
        assertEqual(viewModel.state, current, "preview failure does not replace loaded status state")
    }

    @MainActor
    private static func automaticRefreshUsesSameLoadingRefreshPath() async {
        let loaded = GitStatusLoadState.loaded(GitStatusSummary(
            rootPath: "/tmp/worktree",
            branchName: "main",
            upstreamName: nil,
        ...
        assertEqual(observedLoading, true, "automatic refresh exposes loading state before service returns")
