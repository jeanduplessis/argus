...
struct SessionReconciliationTests {
    static func main() throws {
        ...
        assertEqual(restoredProject.workspaceIds, [validWorkspaceId], "stale refs are removed and valid order is preserved")
        ...
        assertEqual(rereconciled.projects.map(\.workspaceIds), reconciled.projects.map(\.workspaceIds), "reconciliation is idempotent for project membership")
        assertEqual(rereconciled.workspaces.map(\.projectId), reconciled.workspaces.map(\.projectId), "reconciliation is idempotent for workspace membership")
    }
