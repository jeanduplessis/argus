...
struct ExistingBranchPickerTests {
    static func main() async throws {
        ...
        try "hello".write(to: seed.appendingPathComponent("README.md"), atomically: true, encoding: .utf8)
        ...
        try "feature".write(to: seed.appendingPathComponent("feature.txt"), atomically: true, encoding: .utf8)
        ...
        try run("git", ["clone", "--single-branch", "--branch", "main", origin.path, repo.path], cwd: temp.path)
        try run("git", ["config", "user.email", "test@example.com"], cwd: repo.path)
        try run("git", ["config", "user.name", "Test User"], cwd: repo.path)

        let localBranches = try capture("git", ["branch", "--all", "--format=%(refname:short)"], cwd: repo.path)
        assertFalse(localBranches.contains("origin/feature/unfetched"), "test branch starts as an unfetched remote branch")

        let service = WorktreeService()
        let available = try await service.listAvailableBranches(repositoryPath: repo.path)
        assertTrue(available.contains("origin/feature/unfetched"), "available branches include remote heads not present in local remote-tracking refs")
        ...
        assertTrue(checkedOutBranch == "feature/unfetched", "unfetched remote head opens as a local tracking branch")
    }
    ...
    private static func capture(_ executable: String, _ args: [String], cwd: String) throws -> String {
        let process = Process()
        ...
        guard process.terminationStatus == 0 else {
            ...
            throw NSError(domain: "ExistingBranchPickerTests", code: Int(process.terminationStatus), userInfo: [NSLocalizedDescriptionKey: "\(executable) \(args.joined(separator: " ")) failed: \(detail)"])
        }
        return output
    }
