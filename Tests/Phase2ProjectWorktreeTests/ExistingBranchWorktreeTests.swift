...
struct ExistingBranchWorktreeTests {
    static func main() async throws {
        ...
        try "remote".write(to: repo.appendingPathComponent("remote.txt"), atomically: true, encoding: .utf8)
        ...
        assertTrue(available.contains("origin/remote-only"), "remote-only branch remains available with remote label")
        ...
        assertEqual(checkedOutBranch, "remote-only", "remote-only worktree is on a local tracking branch, not detached")
    }
    ...
    private static func capture(_ executable: String, _ args: [String], cwd: String) throws -> String {
        ...
        guard process.terminationStatus == 0 else {
            ...
            throw NSError(domain: "ExistingBranchWorktreeTests", code: Int(process.terminationStatus), userInfo: [NSLocalizedDescriptionKey: "\(executable) \(args.joined(separator: " ")) failed: \(detail)"])
        }
        return output
    }
