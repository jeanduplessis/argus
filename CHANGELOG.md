...
- Pi no longer waits for Argus status delivery before showing a submitted message. The integration now drains socket responses instead of waiting for a 1.5-second timeout, and running-status updates no longer block prompt processing. Enable the updated Pi integration in Settings, then restart Pi or run `/reload` to use the fix. ([98039d0](https://github.com/jeanduplessis/argus/commit/98039d0d78f8f228a1f21b187afed7989773c731))
